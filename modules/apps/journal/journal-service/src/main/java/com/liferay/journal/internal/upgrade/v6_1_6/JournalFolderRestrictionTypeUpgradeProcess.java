...
public class JournalFolderRestrictionTypeUpgradeProcess extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		String sql = StringBundler.concat(
			"UPDATE JournalFolder jf INNER JOIN Group_ g ON jf.groupId = ",
			"g.groupId SET jf.restrictionType = ? WHERE g.classNameId = ? AND ",
			"jf.restrictionType = ?");
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(
			...
			preparedStatement.setInt(
				3, JournalFolderConstants.RESTRICTION_TYPE_WORKFLOW);
